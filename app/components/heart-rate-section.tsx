...
import { syncAppHint, type WatchType } from "@/lib/watch-config";
...
export function HeartRateSection({
  ...
  watchType: WatchType;
  showDeviceBattery?: boolean;
  showRestingHeartRate?: boolean;
  showLiveHeartRate?: boolean;
  showSteps?: boolean;
}) {
  const latest = days[0];
  const hint = syncAppHint(watchType);
        ...
        <div className="space-y-4">
          ...
          {days.length === 0 ? (
            <p className="text-sm text-zinc-500">
              No heart rate data for this range. {hint}, then refresh.
            </p>
          ) : (
