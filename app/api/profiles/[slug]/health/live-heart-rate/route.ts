...
export async function GET(
  ...
  try {
    if (mode === "day" || mode === "hour") {
      ...
      if (mode === "day") {
        const data = await fetchHeartRateDayChartCached(slug, { dateKey, start, end });
      ...
      const data = await fetchHeartRateHourChartCached(slug, {
        dateKey,
        hour,
        start,
        end,
    ...
    const data = await fetchHeartRateWindowChartCached(slug, { windowHours: hours, end });
    return NextResponse.json(data);
  } catch (err) {
